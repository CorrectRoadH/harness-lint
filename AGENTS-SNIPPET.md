...
- For Markdown rules, always write `language markdown(block)` (bare `language markdown` selects the inline grammar and block patterns never match), and match AST nodes such as `atx_heading()` / `fenced_code_block()` instead of multi-token literal snippets. GritQL cannot see YAML frontmatter fields, code inside fences, or inline-within-block structure — do not create a rule that needs those; keep such constraints in agent instructions.

...
6. 运行 `harness-lint rule verify <rule-id>`，证明 Bad 示例会被规则命中。
7. 运行 `harness-lint check --all --rule <rule-id>`，确认新规则命中了预期文件。不要通过给 `check` 传路径来模拟规则范围；如果规则只应该作用于部分文件，必须在 GritQL 中用 `$filename` 表达。
8. 运行 `harness-lint check --changed` 执行 lint，确认规则能被加载并按预期工作。
